...
$uploadsdir=$upload_dir['basedir'];
	...
	function amz_reviews_check_nonce() {
		// Check if our nonce is set.
		if ( ! isset( $_POST['amz_reviews_box_nonce'] ) ) { return false; }
		// Verify that the nonce is valid.
		if ( ! wp_verify_nonce( $_POST['amz_reviews_box_nonce'], 'amz_reviews_check_nonce' ) ) { return false; }
...
<?php
	...
	wp_nonce_field( 'amz_reviews_check_nonce', 'amz_reviews_box_nonce' );
?>
...
$res[0]["description"]= '<img src="'.$uploadsurl.'/amz_images/'.$img1.'" style="float:left; margin:4px; height: 250px !important;" /><img src="'.$uploadsurl.'/amz_images/'.$img.'" style="float:right; margin:4px; height: 250px !important;" />'.$res[0]["description"];
	...
	if( amz_post_exists(wp_strip_all_tags($res[0]["title"])) == 0){
		if(amz_reviews_check_nonce()===true){
			...
			update_post_meta($id, 'amz_url', esc_url($res[0]["url"]));
			...
			echo '<br>Amazon Product successfully added to the Reviews section'.post_exists(wp_strip_all_tags($res[0]["title"]));
		}else{
			echo '<br>nonce error.';
		}
	}else{
